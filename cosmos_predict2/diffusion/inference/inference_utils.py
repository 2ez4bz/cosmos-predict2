...
def load_tokenizer_model(
    model: DiffusionT2WModel, tokenizer_dir: str, load_mean_std: bool = False, mean_std_path: str = ""
):
    if load_mean_std:
...
def resize_input(video: torch.Tensor, resolution: list[int]):
    ...
    Args:
        video (th.Tensor): Input video tensor of shape (T, C, H, W).
        resolution (list[int]): Target resolution [H, W].

    Returns:
        th.Tensor: Resized and cropped video tensor of shape (T, C, target_H, target_W).
    """
    ...
    video_resized = transforms_F.resize(video, resizing_shape)
    video_cropped = transforms_F.center_crop(video_resized, resolution)
...
def read_and_process_image(img_path: str, resolution: list[int], num_video_frames: int, resize: bool = True):
    ...
    Returns:
        th.Tensor: Processed video tensor of shape (1, C, T, H, W).
    ...
    # Convert to tensor
    img = transforms_F.to_tensor(img)
...
def generate_world_from_video_i4(
    model: DiffusionV2WModel,
    is_negative_prompt: bool,
    data_batch: dict,
    guidance: float,
    ...
    sample = model.generate_samples_from_batch_i4(
        data_batch,
