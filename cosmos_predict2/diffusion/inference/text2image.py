...
from cosmos_predict2.diffusion.inference.inference_utils import add_common_arguments, validate_args
from cosmos_predict2.diffusion.inference.world_generation_pipeline import DiffusionText2ImageGenerationPipeline
from cosmos_predict2.utils import log, misc
from cosmos_predict2.utils.io import read_prompts_from_file, save_image
...
def demo(args):
    ...
    - Saving the generated images and corresponding prompts to disk
    ...
    pipeline = DiffusionText2ImageGenerationPipeline(
    ...
    os.makedirs(args.image_save_folder, exist_ok=True)
    for i, input_dict in enumerate(prompts):
        ...
        if generated_output is None:
            ...
            continue
        image, prompt = generated_output

        if args.batch_input_path:
            image_save_path = os.path.join(args.image_save_folder, f"{i}.jpg")
            prompt_save_path = os.path.join(args.image_save_folder, f"{i}.txt")
        else:
            image_save_path = os.path.join(args.image_save_folder, f"{args.image_save_name}.jpg")
            prompt_save_path = os.path.join(args.image_save_folder, f"{args.image_save_name}.txt")

        save_image(image, image_save_path, quality=95)

        # Save prompt to text file alongside image
        with open(prompt_save_path, "wb") as f:
            f.write(prompt.encode("utf-8"))

        log.info(f"Saved image to {image_save_path}")
