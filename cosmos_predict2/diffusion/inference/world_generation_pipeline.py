...
from cosmos_predict2.diffusion.model.model_t2w_multiview import DiffusionMultiviewT2WModel
...
# from cosmos_predict2.diffusion.prompt_upsampler.text2world_prompt_upsampler_inference import (
...
from cosmos_predict2.utils.base_world_generation_pipeline import BaseWorldGenerationPipeline

MODEL_NAME_DICT = {
    "Cosmos-Predict2-2B-Video2World": "Cosmos_Predict2_2B_Video2World",
    "Cosmos-Predict2-14B-Video2World": "Cosmos_Predict2_14B_Video2World",
    "Cosmos-Predict2-2B-Text2Image": "Cosmos_Predict2_2B_Text2Image",
    "Cosmos-Predict2-14B-Text2Image": "Cosmos_Predict2_14B_Text2Image",
}
    ...
    def generate(
        ...
        if self.enable_prompt_upsampler:
            ...
            if word_limit_to_skip_upsampler is None or word_count <= word_limit_to_skip_upsampler:
                ...
                log.info(
                    f"Skip prompt upsampler for better robustness because the number of words "
                    f"({word_count}) in the prompt is greater than {word_limit_to_skip_upsampler}"
                )
...
class DiffusionText2ImageGenerationPipeline(BaseWorldGenerationPipeline):
    def __init__(
        self,
        inference_type: str,
        checkpoint_dir: str,
        ...
        Args:
            inference_type: Type of world generation ('text2image' or 'video2world')
            checkpoint_dir: Base directory containing model checkpoints
            checkpoint_name: Name of the diffusion transformer checkpoint to use
            prompt_upsampler_dir: Directory containing prompt upsampler model weights
            enable_prompt_upsampler: Whether to use prompt upsampling
        ...
        assert inference_type in [
            "text2image",
            "video2world",
        ], "Invalid inference_type, must be 'text2image', 'video2world'"
    ...
    def _run_model(
        ...
        # # Get image batch
        # data_batch_image = get_sample_batch(
        #     num_frames=1,
        #     height=self.height,
        #     width=self.width,
        #     batch_size=1,
        # )
        # data_batch_image["t5_text_embeddings"] = embedding.to(dtype=torch.bfloat16).cuda()
        # data_batch_image["neg_t5_text_embeddings"] = negative_prompt_embedding.to(dtype=torch.bfloat16).cuda()

        # data_batch_image["image_size"] = torch.Tensor([self.height, self.width, self.height, self.width]).to(
        ...
        # sample = generate_image_from_text(
        #     model=self.model,
        #     state_shape=None,
        #     is_negative_prompt=True if negative_prompt_embedding is not None else False,
        #     data_batch=data_batch_image,
        #     guidance=self.guidance,
        #     num_steps=self.num_steps,
        #     seed=self.seed,
        # )
    ...
    def generate(
        ...
        Args:
            prompt: Text description of desired video
            negative_prompt: Optional text to guide what not to generate
            word_limit_to_skip_upsampler: Skip prompt upsampler for better robustness
            if the number of words in the prompt is greater than this value
        Returns:
            tuple: (
                Generated video frames as uint8 np.ndarray [T, H, W, C],
                Final prompt used for generation (may be enhanced)
